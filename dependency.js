...
var width = 1000
var height = 768
var yscale_performancebar = d3.scale.linear().domain([0, 1]).rangeRound([boxHeight / 2, -boxHeight / 2])
...
d3.json(jsonFile, function(error, root) {
    ...
    newNodes.attr("class", "node").attr("id", function(d) {
        return d.name
    }).attr("transform", function(d) {
        return "translate(" + d.y + "," + d.x + ")";
    }).append("rect").attr('class', 'nodebox').attr("x", -boxWidth / 2).attr("y", -boxHeight / 2).attr("width", boxWidth).attr("height", boxHeight)
    newNodes.append("rect").attr('id', 'performancebar').attr("x", boxWidth / 2 * 1.05).attr("width", boxWidth / 10).style("fill", "red").style("stroke", "red").attr("y", boxHeight / 2).attr("height", 0)

    // Highlight node when we mouse-over
    newNodes.on("mouseover", function() {
        thisNode = d3.select(this)
        thisNodeCol = thisNode.select(".nodebox").style("stroke")
        thisNode.selectAll(".nodebox").transition().duration(250).style("fill", thisNodeCol)
        //              .style("opacity", 0.6)
        svg.selectAll(".link").transition().duration(250).style("stroke", thisNodeCol).style("stroke-width", getLinkWidthClass)
    })
    newNodes.on("mouseout", function() {
        d3.select(this).selectAll(".nodebox").transition().duration(250).style("fill", null).style("opacity", null)
        svg.selectAll(".link").transition().duration(250).style("stroke", null).style("stroke-width", function(d) {
            // return 5E-3 * d.target.params.nKPsForThisNode
            return d.target.params.nKPsForThisNode
        })
    })
    //Add node titles
    newNodes.append("text").attr("id", "nodetitle").attr("class", "nodeTitle").attr("y", -boxHeight / 2 + fontSize + 2 * lineSpace).attr("text-anchor", "middle")

    //Add node body text (for f1 score)
    newNodes.append("text").attr("text-anchor", "middle").attr("class", "nodeText").attr("id", "f1Text").attr("y", -boxHeight / 2 + 2 * fontSize + 3 * lineSpace)

    newNodes.append("g").attr("class", "confusionmatrix").attr("id", "confusionmatrix").selectAll("g").data(function(d) {
        return d.params.confusionmatrix
    }).enter().append("g").attr("class", "rows").attr("transform", function(d, i) {
        return "translate(" + (-15) + "," + (-boxHeight / 2 + (i + 3) * fontSize + (i + 4) * lineSpace) + ")";
    }).selectAll("g").data(function(d) {
        return d
    }).enter().append("g").attr("class", "columns").attr("transform", function(d, i) {
        return "translate(" + i * 30 + ",0)";
    }).append("text").attr("text-anchor", "middle").attr("class", "nodeText")

    //ENTER + UPDATE: Update all nodes with new attributes
    existingNodes.select('#performancebar').transition().duration(1000).attr("y", function(d) {
        return yscale_performancebar(d.params["f1-score"])
    }).attr("height", function(d) {
        return boxHeight / 2 - yscale_performancebar(d.params["f1-score"])
    })
    existingLinks.transition().duration(1000).style("stroke-width", function(d) {
        // return 5E-3 * d.target.params.nKPsForThisNode
        return d.target.params.nKPsForThisNode
    })

    existingNodes.select("#nodetitle").text(function(d) {
        return d.name.split("_").slice(-1)
    })
    existingNodes.select("#f1Text").text(node1Text)

    // Update confusion matrix
    existingNodes.select("#confusionmatrix").selectAll(".rows").data(function(d) {
        return d.params.confusionmatrix
